...
import {
  createFile, getMyFiles, renameFile, deleteFile,
  recycleFile, restoreFile
...
export const Component = () => {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "40px auto" }}>
      ...
      {tab === "my" && (
        <div style={{ margin: 12 }}>
          <input value={fileName} onChange={e => setFileName(e.target.value)} placeholder="新文件名" />
          ...
          <li key={f.id}>
            {editId === f.id ? (
              <>
                ...
                <Button onClick={() => { setEditId(""); setEditName(""); }}>取消</Button>
              </>
            ) : (
              <>
                <span>{f.fileName}</span>
                <Button onClick={() => { setEditId(f.id); setEditName(f.fileName); }}>重命名</Button>
                {tab === "my" ? (
                  <>
                    <Button onClick={() => handleRecycle(f.id)}>回收</Button>
                    <Button onClick={() => handleDelete(f.id)} style={{ color: "red" }}>彻底删除</Button>
                  </>
                ) : (
                  <>
